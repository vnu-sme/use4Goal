...
public final class Bpmn2DiagramNode extends PlaceableNode implements ToolTipProvider {
    ...
    public Bpmn2DiagramNode(Bpmn2Node node, Bpmn2DiagramOptions opt, Font font) {
        this.id = node.id;
    ...
    public Bpmn2DiagramNode(String id, String label, ContainerKind kind, int w, int h,
                            Bpmn2DiagramOptions opt, Font font) {
        this.id = id;
        this.label = label;
        this.node = null;
    ...
    private void init(Bpmn2DiagramOptions opt, Font font, int w, int h) {
    ...
    @Override public boolean isResizable() { return containerKind != ContainerKind.NONE || node == null || node.kind != Bpmn2NodeKind.START_EVT; }
    ...
    @Override protected String getStoreType() { return "Bpmn2Node"; }
    ...
    private void paintMarker(Graphics2D g) {
        ...
        } else if (node.kind == Bpmn2NodeKind.START_EVT) {
            paintEventIcon(g, false);
        } else if (node.kind == Bpmn2NodeKind.GATEWAY) {
            paintGatewayMarker(g);
        } else if (node.kind == Bpmn2NodeKind.SUBPROCESS) {
    ...
    private void paintEventIcon(Graphics2D g, boolean filled) {
        if (node.trigger == null || node.trigger == org.vnu.sme.goal.bpmn2.mm.EventTrigger.NONE) return;
    ...
    private void paintGatewayMarker(Graphics2D g) {
        ...
        switch (node.gwKind == null ? org.vnu.sme.goal.bpmn2.mm.GatewayKind.XOR : node.gwKind) {
            case XOR -> {
