...
public final class AclSpecText {
    ...
    public static String render(AclModel model) {
        ...
        model.roles().forEach(value -> renderClassifier(out, "role", value.name(),
                value.parentRoles().stream().findFirst(), value.isAbstract(), value.attributes()));
        model.groups().forEach(value -> renderGroup(out, value));
        model.relations().stream().filter(value -> !value.name().startsWith("member_"))
                .forEach(value -> renderRelation(out, value));
        model.compatibilities().forEach(value -> out.append('\n').append(INDENT)
                .append("compatibility ").append(value.fromRole())
                .append(value.bidirectional() ? " <-> " : " -> ").append(value.toRole()).append(";\n"));
        return out.append("}\n").toString();
    }
