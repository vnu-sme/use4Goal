 ...
 * derived automatically by {@link BpmnUseOclService}.
 */
public final class BpmnUseOclForm extends JDialog {

    private static final Preferences PREFS =
            Preferences.userNodeForPackage(BpmnUseOclForm.class);
    ...
    private final JTextField bpmnField   = new JTextField(50);
    ...
    private void buildUi() {
        ...
        addFileRow(inputPanel, 1, "2. BPMN model (.bpmn2):",  bpmnField,   "bpmn2",  false, false);
    ...
    private void runGenerate() {
        ...
        String bpmnText = bpmnField.getText().trim();
        String outText  = outputField.getText().trim();

        if (aclText.isEmpty() || bpmnText.isEmpty() || outText.isEmpty()) {
            setStatus("Please fill in all three paths.", COLOR_ERR);
            return;
        }

        Path aclPath      = Path.of(aclText);
        Path bpmnPath     = Path.of(bpmnText);
        ...
        new SwingWorker<BpmnUseOclService.Result, Void>() {
            @Override
            protected BpmnUseOclService.Result doInBackground() {
                return BpmnUseOclService.translate(aclPath, bpmnPath, outputFolder);
            }
            ...
            protected void done() {
                ...
                try {
                    BpmnUseOclService.Result result = get();
                    applyResult(result);
                } catch (Exception ex) {
    ...
    private void applyResult(BpmnUseOclService.Result result) {
