...
public final class BpmnForm extends JDialog {
    private static final Preferences PREFS = Preferences.userNodeForPackage(BpmnForm.class);
    ...
    private void open(OpenTarget target) {
        ...
        try {
            BpmnCompiler.Result result = BpmnCompiler.compile(Path.of(path));
            if (!result.ok()) {
                showErrors(result.errors());
                status("Load failed.", C_ERR);
                return;
            }
            BpmnModel model = result.model();
            Path source = Path.of(path);
            if (target == OpenTarget.POPUP_WINDOW) {
                BpmnView.openPopupWindow(mainWindow, model, source);
            } else {
                BpmnView.openUseDesktop(mainWindow, model, source);
            }
