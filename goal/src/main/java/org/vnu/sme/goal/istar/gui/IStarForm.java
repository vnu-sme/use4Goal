...
public final class IStarForm extends JDialog {
    ...
    private void open(OpenTarget target) {
        ...
        try {
            IStarCompiler.Result result = IStarCompiler.compile(Path.of(path));
            if (!result.ok()) {
                showErrors(result.errors());
                status("Load failed.", C_ERR);
                return;
            }
            GoalModel model = result.model();
            Path source = Path.of(path);
            if (target == OpenTarget.POPUP_WINDOW) {
                IStarView.openPopupWindow(mainWindow, model, source);
            } else {
                IStarView.openUseDesktop(mainWindow, model, source);
            }
