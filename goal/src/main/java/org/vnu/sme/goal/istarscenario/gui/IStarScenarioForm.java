package org.vnu.sme.goal.istarscenario.gui;
...
import org.vnu.sme.goal.istarscenario.parser.IStarScenarioCompiler;
import org.vnu.sme.goal.istarscenario.view.IStarScenarioView;
 ...
 * target model's SR diagram annotated with satisfaction badges via {@link IStarScenarioView}.
 */
...
public final class IStarScenarioForm extends JDialog {
    ...
    private static final String PREF_KEY = "istarscenario.lastFile";
    ...
    private void open() {
        ...
        try {
            IStarScenarioCompiler.Result result = IStarScenarioCompiler.compile(Path.of(path));
            if (!result.ok()) {
    ...
    private void printReport(IStarScenarioCompiler.Result result) {
        appendResult("Scenario : " + result.scenario().name());
        appendResult("Model    : " + result.modelFile());
        appendResult("Instances: " + result.markings().size()
                + (result.markings().size() == 1 && result.markings().containsKey(IStarScenarioCompiler.DEFAULT_INSTANCE)
