...
/**
 * Builds the WF-net-style token semantics for a {@link Bpmn2Model} described in
 * §3.3.2 of doc/paper/conformance-istar-bpmn2.md. {@code Bpmn2Model} has no notion
...
public final class Bpmn2LtsBuilder {
     ...
     */
    public static void validateWellFormed(Bpmn2Model pm) {
    ...
    public static List<BpmnFiring> enabledFirings(Bpmn2Model pm, BpmnMarking m) {
    ...
    private static List<BpmnFiring> firingsFor(Bpmn2Model pm, BpmnMarking m, FlowElement n) {
        ...
        Set<String> produced = out.stream().map(Bpmn2LtsBuilder::arcId)
    ...
    private static List<BpmnFiring> joinFirings(BpmnMarking m, FlowElement n, List<SequenceFlow> in,
        ...
        if (type == GatewayKind.AND) {
            Set<String> allIncoming = in.stream().map(Bpmn2LtsBuilder::arcId)
                    .collect(Collectors.toCollection(LinkedHashSet::new));
    ...
    private static List<SequenceFlow> incoming(Bpmn2Model pm, String nodeId) {
        return allFlows(pm).stream().filter(f -> f.target().id().equals(nodeId)).collect(Collectors.toList());
    }

    private static List<SequenceFlow> outgoing(Bpmn2Model pm, String nodeId) {
        return allFlows(pm).stream().filter(f -> f.source().id().equals(nodeId)).collect(Collectors.toList());
    }

    private static List<SequenceFlow> allFlows(Bpmn2Model pm) {
