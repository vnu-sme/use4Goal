 ...
 * (Caballero-Villalobos), instantiated with the token semantics of {@link Bpmn2LtsBuilder}
...
public final class ProductLts {
    ...
    private final Bpmn2Model pm;
    private final ConformanceMapping map;

    public ProductLts(GoalModel gm, Bpmn2Model pm, ConformanceMapping map) {
        this.gm = gm;
        this.pm = pm;
        this.map = map;
    }
    ...
    public List<Transition> successors(ProductState s) {
        ...
        for (BpmnFiring firing : Bpmn2LtsBuilder.enabledFirings(pm, s.bpmn())) {
            BpmnMarking nextBpmn = Bpmn2LtsBuilder.fire(s.bpmn(), firing);
