package org.vnu.sme.goal.iscn.gui;
 ...
 * target model's SR diagram annotated with satisfaction badges via {@link org.vnu.sme.goal.iscn.view.IStarScenarioView}.
 */
...
public final class IStarScenarioForm extends JDialog {
    ...
    private void open() {
        ...
        try {
            org.vnu.sme.goal.iscn.parser.IStarScenarioCompiler.Result result = org.vnu.sme.goal.iscn.parser.IStarScenarioCompiler.compile(Path.of(path));
            if (!result.ok()) {
            ...
            printReport(result);
            org.vnu.sme.goal.iscn.view.IStarScenarioView.openAllInstances(mainWindow, result);
    ...
    private void printReport(org.vnu.sme.goal.iscn.parser.IStarScenarioCompiler.Result result) {
        ...
        appendResult("Instances: " + result.markings().size()
                + (result.markings().size() == 1 && result.markings().containsKey(org.vnu.sme.goal.iscn.parser.IStarScenarioCompiler.DEFAULT_INSTANCE)
        ...
        for (var entry : result.markings().entrySet()) {
            String label = entry.getKey().equals(org.vnu.sme.goal.iscn.parser.IStarScenarioCompiler.DEFAULT_INSTANCE) ? "(default)" : entry.getKey();
            appendResult("── " + label + " ──");
