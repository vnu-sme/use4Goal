...
import org.tzi.use.uml.ocl.expr.ExpInstanceConstructor;
...
public class FilmstripExpressionVisitor implements ExpressionVisitor {
	...
	public void visitObjOp(ExpObjOp exp) {
		...
		if (!(exp.getOperation().cls() instanceof MClass)) {
			throw new TransformationException(
					"Filmstrip transformation requires a class-owned operation");
		}
		MOperation op = mc.mapClass((MClass) exp.getOperation().cls())
				.operation(exp.getOperation().name(), true);
		if(op == null){
	...
	public void visitInstanceConstructor(ExpInstanceConstructor exp) {
		Type mappedType = mc.mapType(exp.type());
		if (!(mappedType instanceof MClassifier)) {
			throw new TransformationException(
					"Could not map instance-constructor classifier " + exp.type());
		}
		try {
			Expression mapped = new ExpInstanceConstructor((MClassifier) mappedType,
					processSubExpressionArray(exp.getArguments()));
			copyExpressionDetails(exp, mapped);
			elements.push(mapped);
		} catch (ExpInvalidException ex) {
			throw new TransformationException("ExpInstanceConstructor", ex);
		}
	}
