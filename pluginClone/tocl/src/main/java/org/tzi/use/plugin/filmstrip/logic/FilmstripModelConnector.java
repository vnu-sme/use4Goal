...
public class FilmstripModelConnector implements MModelConnector {
	...
	public MAttribute mapAttribute(MAttribute attr) {
		MAttribute a = mapClass(requireClass(attr.owner())).attribute(attr.name(), false);
	...
	public MOperation mapOperation(MOperation operation) {
		MOperation op = mapClass(requireClass(operation.cls())).operation(operation.name(), false);
	...
	private MClass requireClass(MClassifier classifier) {
