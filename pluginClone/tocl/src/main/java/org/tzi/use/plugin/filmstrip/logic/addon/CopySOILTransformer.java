...
public class CopySOILTransformer {
	...
	public void transformSoilOperations(){
		for(MClass cls : model.classes()){
			for(MOperation op : cls.operations()){
				if(!op.hasStatement() || !(op.cls() instanceof MClass)
						|| FilmstripUtil.isFilmstripClass((MClass) op.cls())){
					continue;
				}
	...
	private MStatement[] createCopyPrefix(MOperation op, List<VarDecl> vars){
		...
		if (!(op.cls() instanceof MClass)) {
			throw new UnsupportedOperationException(
					"Filmstrip SOIL transformation requires a class-owned operation");
		}
		MClass operationClass = (MClass) op.cls();
		
		MClass snapshot = model.getClass(FilmstripModelConstants.SNAPSHOT_CLASSNAME);
		MClass opCls = model.getClass(FilmstripModelConstants.makeOpCName(operationClass.name(), op.name()));
		...
		MNavigableElement src = snapshot.navigableEnd(operationClass.nameAsRolename());
		MNavigableElement dst = operationClass.navigableEnd(FilmstripModelConstants.makeRoleName(FilmstripModelConstants.SNAPSHOT_CLASSNAME));
		Expression selfSnapshot;
		try {
			selfSnapshot = new ExpNavigation(new ExpVariable("self", operationClass), src, dst, Collections.<Expression>emptyList());
