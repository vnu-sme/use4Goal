...
constraints returns [ASTConstraints n]
:
    { $n = new ASTConstraints(); }
    (
        inv=invariant   { $n.addConstraint($inv.n); }
        | pp=prePost    { $n.addPrePost($pp.n); }
    )*
    EOF
    ;
...
//      ;
/* ------------------------------------
Name: Invariants for an instance of a class
...
/* ------------------------------------
  ...
  (existential)? inv <identifier>: <expression>

  Example:
    1. Standard invariant clause
        inv: self.age > 0
    2. Standard invariant clause with identifier
       inv myInv: self.age > 0
    3. Existential invariant clause (with or without identifier like standard invariant clause)
       existential inv <identifier>: self.children->exists(c | c.age > 0)
*/
...
/* ------------------------------------
  ...
  Example:
      1. Standard pre clause
          pre: self.age > 0
      2. Standard pre clause with identifier
          pre myPre: self.age > 0
      3. Post clause with identifier
          post myPost: self.age > 0
      4. Post clause
          post: self.age > 0
*/
...
/* ------------------------------------
  ...
  Example:
    Tuple(a: Integer, b: String)
    Tuple(name: String, age: Integer, isActive: Boolean)
    Tuple(id: OclAny, price: Real, description: String)
*/
...
/* ------------------------------------
  ...
  Example:
    ...
    price: Real
    id: OclAny
  Each example follows the pattern of an identifier (id) followed by
  a colon (:) and a type (type).
*/
