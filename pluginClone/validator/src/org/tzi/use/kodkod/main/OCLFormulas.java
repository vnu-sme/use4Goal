...
public class OCLFormulas {
	...
	private Formula stdOpForm(org.tzi.use.uml.ocl.expr.Expression expr){
		...
		for(int i = 0; i < eSO.args().length; i++){
			...
			case VARIABLE:
				...
				if(checkIntExpr(eSO.opname()) && eV.type().isTypeOfInteger()){
					eSOExps[i] = null;
			...
			case STDOP:
				...
				if(checkStdExp(eeSO.opname())){
					if(checkMathExpr(eeSO.opname())){
						if(eeSO.args()[0].type().isTypeOfInteger()){
							eSOForms[i] = null;
							eSOExps[i] = null;
							eSOIntExps[i] = getIntExpStdOp(eSO.args()[i]);
						}else{
							if(eeSO.opname().equals("-") && !eeSO.args()[0].type().isTypeOfReal()){
								eSOForms[i] = null;
			...
			case ATTROP:
				...
				if(checkIntExpr(eSO.opname()) && eAO.type().isTypeOfInteger()){
					eSOExps[i] = null;
		...
		if(eSO.opname().equals("or")){
			...
			if(intExp){
				...
				if(eSOExps[0]!=null && eSOExps[1]!=null){
					if(eSO.args()[0].type().isTypeOfString() || eSO.args()[0].type().isTypeOfReal()){
						if(unsolve==false){
							unsolve = true;
							negExp = expr;
						}
	...
	private Formula letForm(org.tzi.use.uml.ocl.expr.Expression expr){
		...
		case IF:
			...
			intExp = eI.type().isTypeOfInteger();
			if(intExp){
	...
	private Formula attrOpForm(org.tzi.use.uml.ocl.expr.Expression expr){
		...
		if(eAO.attr().type().isTypeOfBoolean() && objExp!=null){
