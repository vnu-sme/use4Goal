...
public class AssociationEdge extends EdgeBase  {

    protected boolean isDashed;

    public AssociationEdge(PlaceableNode source, PlaceableNode target, String name, DiagramView diagram , boolean isDashed) {
    ...
    protected void drawAssociationKind( Graphics g, Point2D p2d1, Point2D p2d2 ) {
        ...
        try {
            if (!isDashed) {
                ...
                DirectedEdgeFactory.drawDirectedEdge( g, p2.x, p2.y , p1.x, p1.y, false);
            }
        } catch ( Exception ex ) {
            ex.printStackTrace();
        }
